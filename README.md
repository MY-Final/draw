...
<img src="./docs/hero.png" alt="AI 绘画工作台界面预览" width="820">
...
- **标准 images 协议**:文生图走 `images/generations`,带参考图自动走 `images/edits`(OpenAI 兼容)。
...
<div align="center">
  <img src="./docs/demo.gif" alt="画图全过程演示" width="720">
</div>
...
首次使用:左侧「新建」一个接口预设 → 填 Base URL / API Key / 模型 → 「测试连接」→ 保存 → 输入描述 → 生成。
...
适配层  :统一 generate();无参考图 → generations,有参考图 → edits。
